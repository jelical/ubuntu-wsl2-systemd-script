...
sudo hwclock -s
sudo apt-get install -yqq wget curl vim systemd snapd fuse keychain slim daemonize dbus-user-session fontconfig ca-certificates gnupg lsb-release ntpdate
...
rm -rf ~/.xwin
mkdir -p ~/.xwin
curl -L http://github.com/jelical/ubuntu-wsl2-systemd-script/releases/download/2.0/xwin.tar.gz | tar -C ~/.xwin -zxvf -
